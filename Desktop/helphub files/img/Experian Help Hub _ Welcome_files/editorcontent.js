...
function initSWFObject(replaceDivID,swfWidth,swfHeight,swfPath,autoplay){
	var flashvars = {};
	if (autoplay!=='false'){
...
function grabFeeds(elemId,feedUrl){
	google.load("feeds", "1");
	function initialize() {
		...
		feed.load(function(result) {
		  ...
		  if (!result.error) {
			for (var i = 0; i < result.feed.entries.length; i++) {
			  ...
			  ul.setAttribute("class","feed_post");	
			  var entry = result.feed.entries[i];
			  var attributes = ["title", "link", "publishedDate", "contentSnippet"];
			  for (var j = 0; j < attributes.length; j++) {	
				if(entry[attributes[j]]){ // ignore empty j
					var li = document.createElement("li");
					li.setAttribute("class",attributes[j]);
					if(attributes[j]=='title'){
						if(entry[attributes[1]]){ // if link returned with result
							var link = document.createElement("a");
							link.setAttribute("href",entry[attributes[1]]);
							link.setAttribute("target","blank");
							link.innerHTML = entry[attributes[j]];
							li.appendChild(link);
						}
						else{
							li.innerHTML = entry[attributes[j]];
						}
					}
					else if(j==3){ // if contentSnippet
						li.innerHTML = entry[attributes[j]] + " <a href='" + entry[attributes[1]] +"' target='blank'>Read More</a>";
					}
					else{ 
						li.innerHTML = entry[attributes[j]];
					}
					if(attributes[j]!='link'){//do not print out link as a seperate li
						ul.appendChild(li);
					}
				}
			  }
			...
			container.innerHTML = "<p style='margin-left:10px;'>Feed is currently not available.</p>";
		  }
		});
	}
...
function initVideoGallery1(){ 
	...
	onTemplateReady = function (evt) {
		...
		if(typeof pageUrl.bcvid != "undefined"){
			loadThisVideo("Preview",pageUrl.bcvid,false);
		}
		else{
			loadThisVideo(activePlaylistsArray[0].displayName,activePlaylistsArray[0].mediaIds[0],false); // load video player with first video from first media collection (playlist)
		}
	...
	loadThisVideoDetails = function (playlistTitle, currentVideoId){
		...
		if(currentVideoDto.customFields.experianblogurl != null || currentVideoDto.customFields.facebookurl !=null || currentVideoDto.customFields.twitterurl != null || currentVideoDto.customFields.linkedinurl != null){
			...
			if(typeof medias[m] != "undefined"){
				$("#"+m).modal({
					onShow:function(dialog){
						...
						dialog.container.height(parseInt(medias[m].height)+61);
						initMediaPlayer(medias[m]);   
						$(window).trigger('resize.simplemodal'); // redraw modal with video content
					},
					onClose:function(){
						brightcove.removeExperience(medias[m].name);
						$.modal.impl.close();
					}
				});
			}
