...
var s_account="escukdev"
if(document.location.hostname.match(/\.uat\.|stg1/)) s_account="escukdevdev";
...
var numDots = document.location.hostname.replace(/[^\.]/g,'').length;
if(numDots > 3) { s.fpCookieDomainPeriods = '3' } else { s.fpCookieDomainPeriods = numDots }
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,ppt,docx,xlsx,pptx"
s.linkInternalFilters="javascript:,experian.co.uk,experian.com,uk.experian.com,experianpayments.com,experian.dsvr.co.uk,picosearch.com,creditexpert.co.uk"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"

/* WARNING: Changing any of the below variables will cause drastic
   changes to how your visitor data is collected. Changes should
   only be made when instructed to do so by your account manager */
s.visitorNamespace="experianservicescorp"
s.trackingServer="experianservicescorp.122.2o7.net"
s.dc="122"

/* DynamicObjectIDs config - improves ClickMap tracking*/
...
/* Update UK daylight savings information every year */
s.currentYear=new Date().getFullYear();
switch(s.currentYear){
	case 2013: s.dstStart="03/31/"; s.dstEnd="10/27/"; break;
	case 2014: s.dstStart="03/30/";	s.dstEnd="10/26/"; break;
	case 2015: s.dstStart="03/29/";	s.dstEnd="10/25/"; break;
	case 2016: s.dstStart="03/27/"; s.dstEnd="10/30/"; break;
	default: s.dstStart="03/29/"; s.dstEnd="10/25/";
}
s.dstStart+=s.currentYear;
s.dstEnd+=s.currentYear;

/* Plugin Config */
s.usePlugins=true
function s_doPlugins(s){
	/* Populate campaign variable from query string parameter "cmpid",
	   only populate once */
	if(!s.campaign){s.campaign=s.getQueryParam('cmpid')}
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0)
	
	/* Store internal promotions in eVar2 conversion variable, only
	   populating once */
	if(!s.eVar2){s.eVar2=s.getQueryParam('intcmp')}
	s.eVar2=s.getValOnce(s.eVar2,'s_intcmp',0)

	/* If an internal search term is set in prop1 traffic variable,
	   force it to lowercase, then copy it to eVar1 conversion
	   variable and set a de-duped internal search event */
	if(s.prop1){
	...
	/* Customer tracking - for now we can capture member VS non-member
	   traffic at the purchase confirmation page. Use this on the login
	   page once we have tracking implemented */ 
/*	if(s.pageName=="OrderConfirmationCE"){
/*		s.events="event2"; /* Login event */
/*		s.eVar6="Customer"; /* Set user type as Customer */
/*		s.eVar3="Login"; /* Set self-service type eVar to Login */
/*	} 
/*	s.prop6=s.getAndPersistValue(s.eVar6,'s_prop6',365); /* Check existence of persisting prop6 - user type */ 
/*	if(!s.prop6&&!s.eVar6){
/*		s.prop6="Non Customer";
/*		s.eVar6="D=c6";
/*	}
/*	s.prop6=s.getAndPersistValue(s.eVar6,'s_prop6',365); /* Set persisting prop6 to value of the eVar6 */

	/* Set GMT hour of day in prop7 traffic and eVar7 conversion
	   variables */
	s.prop7=s.getTimeParting('h','+0');
	s.eVar7="D=c7";
		
	/* Set GMT day of week in prop8 traffic and eVar8 conversion
	   variables */
	s.prop8=s.getTimeParting('d','+0');
	s.eVar8="D=c8";
		
	/* Set page view event */
	s.events=s.apl(s.events,'event2',',',2)
	
	/* If error page, prepend "404" to the URL and set as pagename */		
	if(s.pageType=="errorPage"){s.pageName="404: " + s.wd.location;}
	
	/* If no friendly pagename is set, use the pagename plugin */
	// if(!s.pageType&&!s.pageName){s.pageName=s.getPageName()}
		
	/* Set Flash/Silverlight versions in prop9 and prop12 traffic
	   variables */
	s.detectRIA('s_ria','prop9','prop12','','','');

	/* Populate prop11 traffic variable with value of previous
	   page name to correlate with getPercentPageViewed function in
	   prop19 */
	s.prop11=s.getPreviousValue(s.pageName,"pv_p11");
	if (s.prop11){s.prop19=s.getPercentPageViewed();}
	
	/* Calculate time to complete checkout and set in prop14 traffic
	   variable */
	if(s.events.indexOf('scCheckout')>-1){s.ttc='start';}
	if(s.events.indexOf('purchase')>-1){s.ttc='stop';}
	s.prop14=s.getTimeToComplete(s.ttc,'ttc_p14',0);
	
	/* Set SC in eVar15 conversion and prop15 traffic variables, only
	   populating once */
	if(!s.eVar15){s.eVar15=s.getQueryParam('sc')}
	s.eVar15=s.getValOnce(s.eVar15,'sc',0)
	s.prop15="D=v15";

	/* Set BCD in eVar16 conversion and prop16 traffic variables, only
	   populating once */
	if(!s.eVar16){s.eVar16=s.getQueryParam('bcd')}
	s.eVar16=s.getValOnce(s.eVar16,'bcd',0)
	s.prop16="D=v16";
	
	/* Track new/repeat visitors (30 day cookie) in prop17 traffic
	   and eVar17 conversion variables, only populating once */
	s.prop17=s.getNewRepeat();
	s.eVar17="D=c17";
	s.prop17=s.getValOnce(s.prop17,'vo_p17',0);
	s.eVar17=s.getValOnce(s.eVar17,'vo_e17',0);
	
	/* Set number of days since user last visited in prop18 traffic
	   variable */
	s.prop18=s.getDaysSinceLastVisit();
	
	/* Set user agent in prop20 traffic variable */
	s.prop20="D=User-Agent";
	
	/* Set visitor ID in prop21 traffic and eVar5 conversion variables */
	s.prop21=s.eVar5="D=s_vi";
	
	/* Set page name in eVar12 conversion variable */
	if(s.pageName){s.eVar12=s.pageName}
	
	/* Stack internal promotions in eVar13 conversion variable (30 day
	   cookie window, retain duplicate values) */
	s.eVar13=s.crossVisitParticipation(s.eVar2,'v2_stack','30','10','>','purchase',1);
	
	/* Stack campaigns in eVar14 conversion variable (30 day cookie
	   window, retain duplicate values) */
	s.eVar14=s.crossVisitParticipation(s.campaign,'campaign_stack','30','10','>','purchase',1);
	
	/* Stack internal search terms in eVar18 conversion variable (30
	   day cookie window, retain duplicate values) */
	s.eVar18=s.crossVisitParticipation(s.eVar1,'s_intsearch_stack','30','10','>','purchase',1);
	
	/* Stack BU partner values in eVar19 conversion variable (30 day
	   cookie window, retain duplicate values) */
	s.eVar19=s.crossVisitParticipation(s.eVar10,'s_BU_stack','30','10','>','purchase',1);
	
	/* Capture order ID in eVar20 conversion variable */
/*	if(s.purchaseID){s.eVar20=s.purchaseID}
	
	/* Version control, V"year.month" followed by H code version */
	s.prop26="V12.12 | H23.6";

	/* Set up Dynamic Object IDs for ClickMap */
...
/* Utility: split 1.5 - split strings (JS 1.0 compatible) */
s.split=new Function("l","d",""
+"var i,x=0,a=new Array;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x"
+"++]=l.substring(0,i);l=l.substring(i+d.length);}return a");

/* Utility: apl 1.1 - append values to a list*/
...
/* Utility: p_c - make text lower case */
s.p_c=new Function("v","c",""
+"var x=v.indexOf('=');return c.toLowerCase()==v.substring(0,x<0?v.le"
+"ngth:x).toLowerCase()?v:0");

/* Utility: p_gh - get URL of a clicked element*/
s.p_gh=new Function(""
...
/* Utility: Join - join array elements */
...
+";if(v){a.setTime(a.getTime()+e*86400000);s.c_w(c,v,e?a:0);}return v"
+"==k?'':v");

/* Plugin: getPercentPageViewed 1.2 - record the portion of a page
   (0-100%) that the user has viewed */
...
/* Plugin: detectRIA 0.1 - track Flash and Silverlight versions */
...
+"0';if(thismin>15&&thismin<30){mint='15'}if(thismin>30&&thismin<45){"
+"mint='30'}if(thismin>45&&thismin<60){mint='45'}if(thish>=12){ap='PM"
+"';thish=thish-12};if (thish==0){thish=12};if(thisd==6||thisd==0){dt"
+"='Weekend'};var timestring=thish+':'+mint+ap;if(t=='h'){return time"
+"string}if(t=='d'){return dow};if(t=='w'){return dt}}};");

/* Plugin: getDaysSinceLastVisit 1.1.H - time since last visit */
...
/* Plugin: getPreviousValue 1.0 - return previous value of designated
...
/* Plugin: setupDynamicObjectIDs 1.3: Setup Dynamic Object IDs based
   on URL */
